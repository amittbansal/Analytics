...
class startpage(tk.Frame):
    ...
    def cal(self):
        ...
        ro.r('dimension <- dim(matrix0)')
        ro.r('if (dimension[2]>=dimension[1])')
        ro.r('{')
        ...
        ro.r('for (i in 1:10){cat("topic", i,row.names(matrix1)[order(B[,i],decreasing = TRUE)[1:10]],"\n")}')
        
        # else condition when documents are greater than words it executed i.e started with documents

        ro.r('}else{')

        ro.r('matrix1 <- matrix0')
        ...
        ro.r('sum_of_columns <- colSums(x)')
        ro.r('sum_of_matrix <- sum(x)')
        ro.r('for(i in 1:nrow(data.matrix(sum_of_columns))){pword[i]=sum_of_columns[i]/sum_of_matrix')

        #p(d) probability of document
        ro.r('pdoc <- 0')
        ro.r('no_of_documents <- nrow(matrix1)')
        ro.r('for(i in 1:no_of_documents){pdoc[i]=1/no_of_documents}')
        ...
        ro.r('sum_of_rows = rowSums(x)')
        ro.r('pwgd = matrix(rep(0),nrow = rows,ncol = columns)')
        ro.r('for (i in 1:rows){pwgd[i,]=x[i,]/sum_of_rows[i]}')
        ...
        #p(T,D)
        ro.r('A = matrix(rep(0), nrow = nrow(ptgw$membership), ncol = ncol(ptgw$membership))')
        ro.r('for (i in 1:nrow(A)){A[i,] = as.matrix(ptgw$membership[i,]*pdoc[i])}')
        ...
        ro.r('tpwdt <- t(npwgd)')
        ro.r('pwgt <- tpwdt %*% B')
        ro.r('b<-data.frame(pwgt)')
        ro.r('fwrite(b,self.completeName2)')

        #top 10 words 
        ro.r('sink(self.completeName4,append=TRUE)')
        ro.r('write("",self.completeName4, append=FALSE)')
        ro.r('for (i in 1:10){cat ("topic", i,row.names(t(matrix1)[order(pwgt[,i],decreasing = TRUE)[1:10]],"\n")}')
