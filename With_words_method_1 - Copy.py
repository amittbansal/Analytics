...
class startpage(tk.Frame):
    ...
    def cal(self):
        ...
        folder_name1 = "/repeat_words"
        mypath1 = mypath + folder_name1
        if not os.path.isdir(mypath1):
            os.makedirs(mypath1)

        else :
            if os.path.isdir(mypath1):
                shutil.rmtree(mypath1)
                os.makedirs(mypath1) 

        save_path1=mypath1
        folder_name2 = "/wordcloud_images"
        mypath2 = mypath1  + folder_name2
        if not os.path.isdir(mypath2):
            os.makedirs(mypath2)

        else :
            if os.path.isdir(mypath2):
                shutil.rmtree(mypath2)
                os.makedirs(mypath2) 
        save_path2=mypath2        
        ...
        with open(self.completeName5) as openfile:
            for line in openfile:
                line1 = top + str(" ") + str(m) + str(" ") + line
                self.t6.insert("end",line1)
                m = m + 1
                #j = j+1
                selfoname = 'repeated_words_topic_{}.txt'.format(k)    
                selfcompleteName6 = os.path.join(save_path1,selfoname)         
                k = k+1
                words = line.split()
                for word in words :
                    listabc.append(word)
                ...
                for i in range(0,length):
                    ...
                    ro.r.assign('word',listabc[i])
                    ro.r.assign('selfcompleteName6',selfcompleteName6)
                    ro.r('sink(selfcompleteName6,append=TRUE)')
                    ro.r('a <- replicate(times,word)')
                    ro.r('cat(a,"\n")')
                    ro.r('sink()')
                    times = times -1
                selfoname = 'wordcloud_topic_{}.png'.format(l)    
                self.completeName7 = os.path.join(save_path2,selfoname) 
                ro.r.assign('selfcompleteName7',self.completeName7)
                ro.r('text <- readLines(selfcompleteName6)')
                print (self.completeName7)
                ...
                ro.r('png(selfcompleteName7, width=1280,height=800)')
                ro.r('wordcloud(words = d$word, freq = d$freq, min.freq = 1, max.words=200, random.order=FALSE, rot.per=0.35, colors=brewer.pal(8, "Dark2"))')            
                l = l+1 
                del listabc[:]
        openfile.close()
