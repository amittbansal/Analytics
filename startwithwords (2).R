...
#Reading the file
mydata <- readLines("/Users/Amit/Desktop/test.txt")
...
sum_of_matrix <- sum(x)
for(i in 1:nrow(data.matrix(sum_of_rows))){pword[i]=sum_of_rows[i]/sum_of_matrix}

#p(d) probability of document
pdoc <- 0
sum_of_columns <- colSums(x)
for(i in 1:nrow(data.matrix(sum_of_columns))){pdoc[i]=sum_of_columns[i]/sum_of_matrix}

#p(w|d)
rows = nrow(x)
columns = ncol(x)
pwgd = matrix(rep(0),nrow = rows,ncol = columns)
for (j in 1:columns){pwgd[i,j]=x[i,j]/sum_of_columns[j]}
...
for (i in 1:nrow(A)){A[i,] = as.matrix(ptgw$membership[i,]*pword[i])}
...
write("","/Users/Amit/Desktop/a.txt", append=FALSE)
for (i in 1:10)
{
